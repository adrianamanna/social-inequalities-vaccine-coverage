...
def load_RES_Gab(E, M, NPI_S, type_res):
    ...
    RES_Gab_r0s = pd.read_pickle(f"./res/PREP_{type_res}_Gab_{E}_{M}_{NPI_S}.pkl")
...
def load_RES(E, M, M_Cij, type_res):
    ...
    # [D] [0: nonvax, 1:vax]
    res_Cij_r0s = pd.read_pickle(f"./res/PREP_all_Cij_{E}_{M_Cij}_{NPI_S}.pkl")
...
def load_RATES(E, M, NPI_S, model):
    RES = pd.read_pickle(f"./res/RATES_{model}_{E}_{M}_{NPI_S}.pkl")
    return RES
...
def des_epi_all(epi_df, C):
    # epi_df = epi_df_.copy()
    epi_df[f"{C}_all"] = epi_df[C].apply(
        lambda x: pd.DataFrame(x).sum(axis=1).to_dict()
    )
